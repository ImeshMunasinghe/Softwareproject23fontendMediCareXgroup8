...
const RequestCard = ({ request, actionLoading, onApprove, onReject }) => {
...
export default function AccountRequests() {
  ...
  const handleReject = async (request) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.message || "Failed");
      await fetchRequests();
      setSuccessMsg("Request rejected. A notification email has been sent.");
    } catch (err) {
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      {/* Inline error banner */}
      {errorMsg && (
        <div className="flex items-center gap-3 bg-red-50 border-2 border-red-200 text-red-800 rounded-xl px-5 py-4 mb-6 text-sm font-medium">
          {errorMsg}
        </div>
      )}
